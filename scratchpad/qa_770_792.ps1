...
# vi (out) -> zh (src) mapping: out N -> zh N+5
$results = @()
for ($vi = 770; $vi -le 792; $vi++) {
    $zh = $vi + 5
    ...
    $outParaCount = $outLines.Count - 1  # minus title line

    $zhLines = $zhText -split "`r?`n" | Where-Object { $_.Trim() -ne "" }
    $zhParaCount = $zhLines.Count - 1  # minus header line
...
$results | Export-Csv -Path "C:\truyen\maphap\scratchpad\qa_770_792.csv" -NoTypeInformation -Encoding UTF8
Write-Host "---"
