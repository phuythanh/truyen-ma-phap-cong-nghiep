...
1. Spawn subagent `self` bằng model `pro` (Gemini 3.5 Pro) để thực hiện dịch chương hoặc cụm chương.
2. **Định dạng file dịch (`chapters_out/{vi_chapter:D4}.md`):**
...
### Bước 3: Chạy QA Tự động & Quản lý (Bằng Main Agent với model Gemini Flash)
Main Agent chạy bằng model Gemini 3.5 Flash sẽ quản lý toàn bộ quy trình và thực hiện chạy script kiểm tra chất lượng bằng PowerShell:
```powershell
