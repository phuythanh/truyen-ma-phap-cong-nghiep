...
* **Cảnh giác khi chạy script tự động (PowerShell/Python):**
  * Trong **PowerShell (5.1 mặc định)**:
    * Tránh dùng pipe (`|`) hoặc redirect (`>`) trực tiếp từ stdout của các lệnh console (như `git show`) mà không thiết lập encoding, vì PowerShell sẽ tự động giải mã thành rác ký tự ANSI. Luôn dùng các phương thức của hệ thống như `[System.IO.File]::WriteAllLines()` hoặc chỉ định tham số `-Encoding UTF8` rõ ràng.
    * **Lưu ý đặc biệt khi truyền câu lệnh trực tiếp qua CLI:** Tránh bọc câu lệnh trong dấu nháy kép `"` (ví dụ: `powershell -Command "..."`) nếu câu lệnh có sử dụng ký hiệu `$` (như các biến PowerShell `$p`, `$_`). Việc bọc nháy kép sẽ khiến shell cha tự động nội suy/mở rộng biến này trước khi truyền cho PowerShell (biến thành chuỗi rỗng), dẫn tới lỗi cú pháp nghiêm trọng (như lỗi `= : The term '=' is not recognized`). **Bắt buộc dùng dấu nháy đơn `'...'`** để giữ nguyên vẹn ký tự câu lệnh khi truyền qua CLI.
