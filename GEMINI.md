...
### Bước 2: Dịch thuật (Gợi ý dùng subagent)
Để tiết kiệm token cho phiên chính và chạy song song (nếu cần dịch batch nhiều chương):
1. Spawn subagent `self` thực hiện dịch chương hoặc cụm chương.
...
### Bước 3: Chạy QA Tự động (Bắt buộc)
Chạy script kiểm tra chất lượng bằng PowerShell:
```powershell
